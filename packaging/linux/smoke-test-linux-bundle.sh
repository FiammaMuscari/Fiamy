...
if grep -Eiq 'error while loading shared libraries|cannot open shared object file|Could not (find|load) the Qt platform plugin|No such file or directory' "${LOG_FILE}"; then
  cat "${LOG_FILE}" >&2
